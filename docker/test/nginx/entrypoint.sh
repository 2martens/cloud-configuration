...
if [ -f "$CERT_PATH" ] && [ -f "$KEY_PATH" ]; then
  ...
  ln -sf /etc/nginx/certificates-available.d/letsencrypt.conf /etc/nginx/certificates-enabled.d/certificates.conf
else
  echo "⚠️ No certificate found — starting Nginx with self-signed TLS fallback"
  ln -sf /etc/nginx/certificates-available.d/selfsigned.conf /etc/nginx/certificates-enabled.d/certificates.conf
fi
